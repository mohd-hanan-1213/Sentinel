...
class Dashboard(QMainWindow):
    def __init__(self, admin_id):
        ...
        delete = QPushButton("Delete selected photo"); delete.clicked.connect(self.delete_photo)
        self.tabs.widget(4).layout().addWidget(view); self.tabs.widget(4).layout().addWidget(delete); self.tabs.currentChanged.connect(lambda _: self.refresh()); self.refresh()
    @staticmethod
    def table(headers):
